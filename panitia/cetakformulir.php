...
if(!isset($_GET['id']))
{
echo "<script>alert('anda belum memilih siswa');document.location.href='index.php'</script>";    
exit();
}
$now=date("M d Y");
$siswa=mysql_query("select * from ppdb_data_siswa,ppdb_biodata where
        ppdb_data_siswa.no_peserta=ppdb_biodata.no_peserta and ppdb_data_siswa.no_peserta='$_GET[id]'")or die(mysql_error());
